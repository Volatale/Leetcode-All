...
class Solution:
    def maxProfit(self, prices: list[int]) -> int:
        ...
        dp: list[list[list[int]]] = [[[0] * 2 for _ in range(3)] for _ in range(2)]
        ...
        for i in range(n - 1, -1, -1):
            for sales in range(3):
                ...
                # * Holding a stock
                dp[i & 1][sales][1] = max(
                    dp[(i + 1) & 1][sales][1],  # * Keep holding
                    dp[(i + 1) & 1][sales + 1][0] + prices[i],  # * Make a sale
                )

                # * Not holding a stock
                dp[i & 1][sales][0] = max(
                    dp[(i + 1) & 1][sales][0],  # * Stay flat
                    dp[(i + 1) & 1][sales][1] - prices[i],  # * Buy a stock
                )
