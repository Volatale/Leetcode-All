...
# ! Note that we only rely on two rows at any given point, so we can use a rolling array
class Solution:
    def isMatch(self, s: str, p: str) -> bool:
        ...
        dp: list[list[bool]] = [[False] * (m + 1) for _ in range(2)]
        ...
        for i in range(1, n + 1):
            ...
            for j in range(1, m + 1):
                ...
                if p[j - 1] == s[i - 1] or p[j - 1] == "?":
                    ...
                    dp[i & 1][j] = dp[i & 1][j - 1] or dp[(i - 1) & 1][j]
...
# * Space: O(m) - We only need to keep two rows in memory at any given point
